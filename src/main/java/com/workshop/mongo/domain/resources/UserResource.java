...
public class UserResource {
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.GET ) // pode ser usado o GetMapping, aqui é so uma alternativa
	public ResponseEntity<UserDTO> findById(@PathVariable String id){
		User obj = service.findById(id);
		return ResponseEntity.ok().body(new UserDTO(obj));
	}
	
	@RequestMapping(method = RequestMethod.POST ) // pode ser usado o PostMapping, aqui é so uma alternativa
	...
	@RequestMapping(method = RequestMethod.DELETE ) // pode ser usado o PostMapping, aqui é so uma alternativa
